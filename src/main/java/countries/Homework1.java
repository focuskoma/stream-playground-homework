...
public class Homework1 {
    ...
    public void streamPipeline3() {
        countries.stream().filter(country -> country.getName().toLowerCase().charAt(0)
    ...
    public double streamPipeline12() {
        return countries.stream().mapToDouble(country -> country.getName().length()).average().orElse(Double.NaN);
    }
    ...
    public void streamPipeline13() {
        countries.stream().filter(country -> country.getArea() == null)
                .flatMap(country -> Stream.of(country.getRegion())).distinct().forEach(System.out::println);
    }
    ...
    public void streamPipeline15() {
        countries.stream().filter(country -> country.getArea() != null && country.getArea().compareTo(BigDecimal.ONE) < 0)
                .map(Country::getName).forEach(System.out::println);
    }
    ...
    public void streamPipeline16() {
        countries.stream().filter(country -> country.getRegion() == Region.EUROPE || country.getRegion() == Region.ASIA)
